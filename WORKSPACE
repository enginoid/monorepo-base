...
################################################################
# rbe_autoconfig (https://releases.bazel.build/bazel-toolchains.html)
################################################################
...
# The protobuf library is a Bazel repository. Its role in the build
# process is to generate the proto files through providing protoc.
#
...
# rules_docker lets us build docker containers.
...
# Gets the docker rules to build containers.
load(
...
# Gets a Docker go images to build go containers.
load(
...
http_archive(
    name = "googleapis",
    build_file = "@bazel_buildfarm//:BUILD.googleapis",
...
http_archive(
    name = "remote_apis",
    build_file = "@bazel_buildfarm//:BUILD.remote_apis",
    patch_args = ["-p1"],
    patches = ["@bazel_buildfarm//third_party/remote-apis:remote-apis.patch"],
...
load("@bazel_buildfarm//3rdparty:workspace.bzl", "maven_dependencies")
maven_dependencies()
